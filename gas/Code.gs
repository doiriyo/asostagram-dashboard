// ============================================================
// asostagram インサイト自動集計 - Google Apps Script v3
// ============================================================
//
// 【概要】
...
function getSetting(key) {
  ...
  if (!sheet) {
    throw new Error('「設定」シートが見つかりません。先にメニューの「📊 asostagram → 初期セットアップ」を実行してください。');
  }
...
function notifyTokenError(errorMessage) {
  ...
  MailApp.sendEmail({
    to: email,
    subject: '【asostagram】アクセストークンの更新に失敗しました',
    body: `asostagramインサイト自動集計のアクセストークン更新に失敗しました。\n\n`
      + `エラー内容: ${errorMessage}\n\n`
